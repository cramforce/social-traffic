...
public class Router extends RouteBuilder {
	...
	public void configure() {
		...
		from(
				"restlet:http://" + listenaddress.toString()
						+ "/traffic/byTwitter/{screenName}?restletMethods=get")
				.bean(this, "byTwitter");
		from(
				"restlet:http://" + listenaddress.toString()
	...
	private String getServer() {
		...
		synchronized (this) {
			...
			final InputStream is = Router.class
					.getClassLoader()
					.getResourceAsStream(
			...
			if (is != null) {
				...
				try {
					...
					doc = db.parse(is);
					version = "Informer("
							+ doc.getElementsByTagName("version").item(0)
									.getTextContent() + ")";
				} catch (Exception e) {
	...
	public void top10(Exchange exchange) {
		...
		StringWriter str = new StringWriter();
		try {
			(new ObjectMapper()).writeValue(str, streamies.top(10));
		} catch (Exception e) {
			e.printStackTrace();
		}
		_out.setBody(str.toString());
